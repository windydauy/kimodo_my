#!/usr/bin/env python3
"""Build dense root-only prefix constraints from raw custom-motion qpos NPZ."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract the first N frames of root trajectory from a raw custom-motion qpos NPZ "
            "and save them as Kimodo root2d constraints."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Number of leading frames (at target FPS) to keep as dense root constraints.",
    )
    parser.add_argument(
        "--target-fps",
        type=float,
...
def main() -> None:
    ...
    total_frames = int(root_positions.shape[0])
    ...
    smooth_root_2d = root_positions[:prefix_frames, [0, 2]]

    # Include heading so root prefix constraints also anchor initial facing direction.
    _, posed_joints, _ = skeleton.fk(local_joint_rots[:prefix_frames], root_positions[:prefix_frames])
    root_yaw = compute_heading_angle(posed_joints, skeleton)
    global_root_heading = torch.stack((torch.cos(root_yaw), torch.sin(root_yaw)), dim=-1)
    ...
    print(f"Saved root prefix constraints: {output_path}")
