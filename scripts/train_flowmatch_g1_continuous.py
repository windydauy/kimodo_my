#!/usr/bin/env python3
"""Continuous Flow-Matching distillation for Kimodo G1 (DDIM teacher trajectory -> FM student)."""
...
def _continuous_teacher_target(teacher_traj: torch.Tensor, teacher_time: torch.Tensor, tau: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Piecewise-linear teacher field from a diffusion trajectory.

    tau in [0,1]: 1=noise side, 0=clean side.
    Returns x_tau and v_teacher (=dx/dtau).
    """
    K = teacher_traj.shape[0]
    B = teacher_traj.shape[1]
    # teacher_traj is stored from noise -> clean, while tau=1 means noise
    # and tau=0 means clean. Convert tau to trajectory index accordingly.
    s = (1.0 - tau[:, 0, 0]) * float(K - 1)
    k = torch.floor(s).to(torch.long).clamp(0, K - 2)
    frac = (s - k.to(s.dtype)).view(B, 1, 1)
    b_idx = torch.arange(B, device=teacher_traj.device)
    x0 = teacher_traj[k, b_idx]
    x1 = teacher_traj[k + 1, b_idx]
    t0 = teacher_time[k].view(B, 1, 1)
    t1 = teacher_time[k + 1].view(B, 1, 1)
    dt = (t1 - t0).clamp(max=-1e-4, min=-1.0)
    x_tau = x0 * (1.0 - frac) + x1 * frac
    v_teacher = (x1 - x0) / dt
    return x_tau, v_teacher


def _smooth_teacher_pair_from_neighborhood(
    raw_neighborhood: torch.Tensor,
    k: torch.Tensor,
    *,
    num_states: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return smoothed states at k and k+1 from raw [k-1,k,k+1,k+2]."""
    prev_raw = raw_neighborhood[:, 0]
    cur_raw = raw_neighborhood[:, 1]
    nxt_raw = raw_neighborhood[:, 2]
    nxt2_raw = raw_neighborhood[:, 3]

    cur_smoothed = 0.25 * prev_raw + 0.5 * cur_raw + 0.25 * nxt_raw
    nxt_smoothed = 0.25 * cur_raw + 0.5 * nxt_raw + 0.25 * nxt2_raw

    cur_is_boundary = (k == 0).view(-1, 1, 1)
    nxt_is_boundary = (k + 1 == num_states - 1).view(-1, 1, 1)
    cur_smoothed = torch.where(cur_is_boundary, cur_raw, cur_smoothed)
    nxt_smoothed = torch.where(nxt_is_boundary, nxt_raw, nxt_smoothed)
    return cur_smoothed, nxt_smoothed


@torch.no_grad()
def _teacher_target_at_tau(
    *,
    teacher: nn.Module,
    diffusion: Diffusion,
    sampler: DDIMSampler,
    ...
    """Construct teacher x_tau and dx/dtau without storing the full trajectory."""
    bsz = gt_x0.shape[0]
    num_states = int(teacher_steps)
    if num_states < 2:
        raise ValueError(f"teacher_steps must be >= 2, got {teacher_steps}.")

    s = (1.0 - tau[:, 0, 0]) * float(num_states - 1)
    k = torch.floor(s).to(torch.long).clamp(0, num_states - 2)
    frac = (s - k.to(s.dtype)).view(bsz, 1, 1)

    if smooth:
        needed_indices = torch.stack(
            [
                (k - 1).clamp(0, num_states - 1),
                k,
                k + 1,
                (k + 2).clamp(0, num_states - 1),
            ],
            dim=1,
        )
    else:
        needed_indices = torch.stack([k, k + 1], dim=1)
    num_needed = int(needed_indices.shape[1])
    raw_neighborhood = gt_x0.new_empty((bsz, num_needed, *gt_x0.shape[1:]))
    filled = torch.zeros((bsz, num_needed), device=gt_x0.device, dtype=torch.bool)

    def capture_state(state_idx: int, state: torch.Tensor) -> None:
        for offset in range(num_needed):
            mask = needed_indices[:, offset] == state_idx
            if bool(mask.any().item()):
                raw_neighborhood[mask, offset] = state[mask]
                filled[mask, offset] = True

    x = torch.randn_like(gt_x0)
    capture_state(0, x)
    use_timesteps, map_tensor = diffusion.space_timesteps(num_states)
    diffusion.calc_diffusion_vars(use_timesteps)
    for step_idx in reversed(range(1, num_states)):
        ...
        capture_state(num_states - step_idx, x)
        if bool(filled.all().item()):
            break

    if not bool(filled.all().item()):
        missing = int((~filled).sum().item())
        raise RuntimeError(f"Failed to capture {missing} teacher trajectory states.")

    if smooth:
        x0, x1 = _smooth_teacher_pair_from_neighborhood(raw_neighborhood, k, num_states=num_states)
    else:
        x0 = raw_neighborhood[:, 0]
        x1 = raw_neighborhood[:, 1]

    dt = gt_x0.new_full((bsz, 1, 1), -1.0 / float(num_states - 1))
    x_tau = x0 * (1.0 - frac) + x1 * frac
    v_teacher = (x1 - x0) / dt
    return x_tau, v_teacher
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train continuous FM student from DDIM teacher trajectory.")
    parser.add_argument(
        "--config",
        type=str,
        default="kimodo/distillation/configs/flowmatch_g1_ddim_teacher100_student20_2gpu_10k.yaml",
...
def main() -> None:
    ...
    teacher_smoothing = str(
        cfg.get("flow_matching", {})
        .get("teacher_velocity_target", {})
        .get("smoothing", "none")
    ).lower() not in {"none", "false", "off", "0", "no"}
    ...
    for global_step in range(start_step, total_steps):
        ...
        # Continuous FM: sample tau~U(0,1), where tau=1 is noise side and tau=0 is clean side.
        tau = torch.rand((gt_x0.shape[0], 1, 1), device=device, dtype=gt_x0.dtype)

        with torch.no_grad():
            with autocast_ctx():
                x_teacher_tau, v_teacher = _teacher_target_at_tau(
                    teacher=teacher,
                    diffusion=diffusion,
                    sampler=sampler,
