#!/usr/bin/env python3
"""Viser comparison: original G1 CSV clip (ghost) vs generated G1 motion (solid)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Viser comparison: G1 CSV ghost vs generated G1 robot.")
    parser.add_argument("--original-csv", required=True, help="Original G1 CSV path.")
    parser.add_argument("--original-fps", type=float, default=120.0, help="FPS of original CSV.")
    parser.add_argument("--generated-csv", required=True, help="Generated MuJoCo qpos CSV path.")
    parser.add_argument("--generated-npz", default=None, help="Optional generated NPZ fallback.")
    ...
    parser.add_argument("--generated-color", type=str, default="255,120,0", help="Generated robot RGB, e.g. 255,120,0")
    parser.add_argument("--ghost-opacity", type=float, default=0.2, help="Opacity for original ghost mesh.")
...
def _load_original_pose_from_csv(csv_path: Path, fps: float, skeleton: G1Skeleton34, xml_path: str) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    root_positions_k = motion["root_positions"].cpu().numpy()
    local_rots_k = motion["local_joint_rots"].cpu().numpy()

    # Convert to qpos in MuJoCo frame for consistent FK visualization path.
    from kimodo.exports.mujoco import MujocoQposConverter

    converter = MujocoQposConverter(skeleton)
    qpos = converter.to_qpos(
        torch.from_numpy(local_rots_k).unsqueeze(0),
        torch.from_numpy(root_positions_k).unsqueeze(0),
        root_quat_w_first=True,
        mujoco_rest_zero=False,
    )
    qpos_np = np.asarray(qpos.squeeze(0).cpu().numpy(), dtype=np.float64)
    pos, rot = _qpos_to_g1_pose_in_kimodo(qpos_np, skeleton, xml_path)
    return pos, rot, float(fps)


def _load_generated_pose(args: argparse.Namespace, skeleton: G1Skeleton34, xml_path: str) -> tuple[np.ndarray, np.ndarray, float, str]:
    ...
    if args.generated_npz is None:
        raise FileNotFoundError("Generated CSV not found and no --generated-npz fallback provided.")
    gen_npz = Path(args.generated_npz)
    if not gen_npz.exists():
        raise FileNotFoundError(f"Generated NPZ not found: {gen_npz}")
...
def main() -> None:
    ...
    original_pos, original_rot, original_fps = _load_original_pose_from_csv(
        Path(args.original_csv), args.original_fps, skeleton, xml_path
    )
    ...
    server.gui.add_markdown(
        f"### G1 CSV Overlay Viewer\n"
        f"- original: `{args.original_csv}` (ghost)\n"
    ...
    while True:
        ...
        ghost.set_pose(torch.from_numpy(original_pos[i0]).float(), torch.from_numpy(original_rot[i0]).float())
        robot.set_pose(torch.from_numpy(generated_pos[i1]).float(), torch.from_numpy(generated_rot[i1]).float())
        time.sleep(1.0 / 60.0)
